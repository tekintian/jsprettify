...
- ✅ **语法验证**：自动验证输出代码的语法正确性
- 🔥 **多种打包方案**：提供三种不同的打包分支满足不同需求

## 🌿 项目分支说明

本项目提供了三个不同的分支，每个分支采用不同的打包技术：

### 1. main 分支 (ncc 打包)
- **打包工具**: `@vercel/ncc`
- **Node.js 版本要求**: >= 16.0.0
- **特点**: 将整个项目打包成单个 JavaScript 文件，体积较小，适合作为模块使用
- **使用场景**: 适合集成到其他 Node.js 项目中

### 2. pkg 分支 (pkg 打包) 
- **打包工具**: `pkg` (版本 2.8.8)
- **Node.js 版本要求**: 无特定要求（已内置运行时）
- **特点**: 打包成独立的可执行文件，无需安装 Node.js 环境即可运行
- **使用场景**: 适合分发给没有 Node.js 环境的用户使用

### 3. sea 分支 (Node.js 原生打包)
- **打包工具**: Node.js 原生 SEA (Single Executable Application)
- **Node.js 版本要求**: >= 20.0.0
- **特点**: 利用 Node.js 原生功能创建单一可执行文件，性能更优
- **使用场景**: 适合追求极致性能和原生体验的用户

### 打包方式对比

| 特性 | ncc (main 分支) | pkg (pkg 分支) | SEA (sea 分支) |
|--------|------------------|------------------|------------------|
| **可执行文件大小** | 小 (~20-30 MB) | 大 (~50-70 MB) | 中等 (~30-40 MB) |
| **运行时 Node.js 版本要求** | 运行时需 >= 16.0.0 | 无需 Node.js (内置) | 运行时需 >= 20.0.0 |
| **执行效率** | 启动速度快 | 启动速度中等 | 启动速度最快 |
| **平台独立性** | 需要 Node.js 环境 | 完全独立 | 需要 Node.js 20+ |
| **部署复杂度** | 需要 Node.js 环境 | 简单复制即可运行 | 需要 Node.js 20+ 环境 |
| **打包速度** | 快 | 中等 | 最慢 |
| **内存使用** | 低 | 高 | 中等 |
| **安全性** | 标准 | 高 (自包含) | 高 (原生) |
| **维护难度** | 中等 | 简单 | 中等 |
| **最佳使用场景** | 模块/库使用 | 终端用户分发 | 性能关键型应用 |

## 📦 安装

### 方式一：使用 npm（针对 main 分支）
...
根据您选择的分支，从 [GitHub Releases](https://github.com/tekintian/jsprettify/releases) 下载对应平台的可执行文件：
...
# 切换到所需分支
git checkout main      # ncc 打包分支
# 或
git checkout pkg       # pkg 打包分支
# 或
git checkout sea       # sea 打包分支
...
# 示例 2：指定输出文件名 (main 分支)
jsprettify bg.min.js beautified.js

# 示例 3：使用不同平台的可执行文件 (pkg/sea 分支)
...
- **main 分支**: Node.js 16.0 或更高版本
- **pkg 分支**: Node.js 任意版本（打包后不依赖）
- **sea 分支**: Node.js 20.0 或更高版本
...
# pkg 分支 - 构建 pkg 版本
npm run build:pkg  # 需要先配置 pkg 相关脚本

# sea 分支 - 构建 SEA 版本
npm run build:sea  # 需要先配置 SEA 相关脚本
```
...
```bash
# 构建应用程序 bundle
npm run build:bundle  # 将应用打包成单个 JS 文件

# 使用 Node.js SEA 功能创建可执行文件
...
- **[pkg](https://github.com/vercel/pkg)**: 将 Node.js 应用打包为可执行文件 (pkg 分支)
- **[@vercel/ncc](https://github.com/vercel/ncc)**: 将 Node.js 应用打包为单个文件 (main 分支)
- **[Node.js SEA](https://nodejs.org/api/single-executable-applications.html)**: 单一可执行应用程序 (sea 分支)
...
- **专业服务**: 我们提供专业的软件定制开发服务，如果您有任何软件开发需求，欢迎通过以上联系方式咨询。
...
A: 
- **main (ncc)**: 体积最小，但需要 Node.js 环境
- **pkg**: 体积较大，但独立运行，无需 Node.js
- **sea**: 体积适中，性能最优，需要 Node.js 20+
