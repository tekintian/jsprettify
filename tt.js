...
async function R(a, b) {
    ...
    try {
        const m = await fetch(a["2"], { method: f, credentials: "include", headers: new Headers(d), body: c })
        if (m.ok) {
            let u = await m.text()
            ;(a.S || function () {})(u)
        }
...
function U() {
    ...
    this.j(chrome.webRequest.onBeforeRequest, this.T, { urls: ["http://*/*", "https://*/*", "ftp://*/*"], types: w }, [
        "requestBody",
    ])
    this.j(chrome.webRequest.onBeforeSendHeaders, this.U, { urls: ["https://*/*", "http://*/*"], types: w }, [
        "requestHeaders",
    ])
    this.j(chrome.webRequest.onHeadersReceived, this.V, { urls: ["<all_urls>"], types: w }, ["responseHeaders"])
...
V.M = function () {
    ...
    chrome.action.setTitle({ title: this.v ? "" : "Download catcher is Off\r\nClick to toggle catching" })
...
V.X = function (a) {
    h || !this.v
        ...
        : this.u != a.finalUrl && this.u != a.url
          ? (this.u = "")
          : ((this.u = ""), chrome.downloads.cancel(a.id), chrome.downloads.erase({ id: a.id }))
}
    ...
    if (this.D) {
        ...
        if (!(118784 < b.length))
            if (a["3"]) (this.G.send(b), (this.i = null))
                ...
                ("POST" != a["1"] && this.C && ((b += "8:" + a["8"] + "\r\n"), (b += "7:" + a["7"] + "\r\n")),
                    this.G.send(b),
                    (this.i = null))
            else
                try {
                    const f = await fetch(a["2"], { method: "HEAD", credentials: "include" })
                    f.ok &&
    ...
    "waiting" == a
        ...
        : "nowaiting" == a
          ...
          : !P(a, "Version") &&
            ...
            ((a = "1" == a.split("=")[1]),
            a != this.F &&
                ((this.F = a), chrome.storage.local.set({ ka: a ? 1 : -1 }, function () {}), this.ga([13, a])))
}
V.ca = function () {
    this.D = !1
    if (this.i) {
...
X.j = function (a) {
    ...
    if ((a = a.split(",")) && a.length)
        for (var c = 0; c < a.length; c++) {
            ...
            d &&
                2 == d.length &&
                ("BANDWIDTH" == d[0].toString().trim() && (b += parseInt(parseInt(d[1]) / 1024) + " Kbps "),
...
X.i = function (a, b) {
    ...
    if (0 != b.length && "#EXTM3U" == b[0].trim()) {
        for (var g = !1, m = !1, u = !1, n = "", p = RegExp("^#(EXT[^\\s:]+)(?::(.*))"), r = 1; r < b.length; r++) {
            var k = b[r].trim()
            k &&
                ("#" == k[0]
                    ? 0 == k.indexOf("#EXT") &&
        ...
        if (e) {
            b = ""
            d && (60 < d && (b += parseInt(d / 60) + " min "), (b += parseInt(d % 60) && parseInt(d % 60) + " sec"))
            var l = { 6: "media", fEx: "ts", 4: "TS File " + b, fDu: b }
            l = M(l, { 1: a["1"], 2: e, tabId: a.tabId, frameId: a.frameId, fS: a["7"], fileName: a.fileName })
            Y(a, l)
            "POST" == l["1"] && S(a, l)
            setTimeout(function () {
                f.h.A(l)
                      ...
                      for (var x = 0; x < c.length; x++)
                          f.h.A(
                              M(
                                  ...
                                  { 1: "GET", 2: c[x]["2"], 6: "hls", fEx: "ts", 4: "TS File " + f.j(c[x].tags) },
                              ),
                          )
                  }, 2500)
                : 0 < d &&
...
function fa(a, b) {
    ...
    if ("application/x-www-form-urlencoded" == e) {
        for (d in c)
            for (e = c[d], d = d.split(" ").map(encodeURIComponent).join("+"), b = 0; b < e.length; b++)
                (a.length && a.push("&"), a.push(d, "=", e[b].split(" ").map(encodeURIComponent).join("+")))
        return a.join("")
    }
...
V.V = function (a) {
    ...
    if ((b = this.m[c])) {
        var e = a.url,
            ...
            u = Q(e)
        if (!u || ("http" != u && "https" != u) || ("GET" != m && "POST" != m)) delete this.m[c]
        else {
                    ...
                    if (200 == a || 206 == a) {
                        ...
                        if (0 !== x)
                            if (
                                ...
                                (e = f.hostname),
                                (f = f.pathname),
                                (m = f.split("/").pop().trim()) && (m = m.split("?").shift().trim()),
                                (b.l = m || ""),
                                (b.s = K(b.l)),
                                ...
                                P(b.l.toLowerCase(), "manif") ||
                                    ...
                                    !(
                                        ...
                                        ((b.R || !g) && da.test(b.g)) ||
                                        ((b.R || "other" == b.type) &&
                                            ((k && !r) || ba.test(p) || (b.g && !B.test(b.g) && !ca.test(b.g))))
                                    ))
                            ) {
                                k =
                                    "vtt" == b.g.toLowerCase() ||
                                ...
                                chrome.tabs.query({ active: !0, currentWindow: !0 }, function (t) {
                                    if (
                                        ...
                                        v["5"] = y && y["2"]
                                        v["4"] = y && y["4"]
                                        B.test(b.g) && (chrome.tabs.remove(t[0].id), (v["6"] = "media"))
                                    }
                                })
                                "POST" == v["1"] && S(b, v)
                                Y(b, v)
...
function Y(a, b) {
    if (a.o) for (var c = 0; c < a.o.length; c++) N(a.o[c].name.toLowerCase(), "x-") && (b[a.o[c].name] = a.o[c].value)
}
V.U = function (a) {
...
V.T = function (a) {
    if (!(0 > a.tabId || 0 > a.frameId))
        if ("ftp" == Q(a.url)) {
            ...
            ((b = a.requestId),
                (c = this.m[b] || { id: b, 2: a.url, tabId: a.tabId, frameId: a.frameId }),
